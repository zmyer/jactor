...
public class TestJProactor {
    ...
    Demultiplexor m_demultiplexors[] = null;
    ...
    public TestJProactor() throws Exception {
    }
    ...
    public void startAcceptor() throws Exception {
        ...
        Acceptor a = new Acceptor(m_demultiplexors[0], addr, m_srvFactory);
    ...
    public void startAutoClients() throws Exception {
        ...
        int idx = m_numDemux - 1;

        Connector c = new Connector(m_demultiplexors[idx], m_clnFactory);
    ...
    public void startManualClient() throws Exception {
        ...
        int idx = m_numDemux - 1;
        Connector c = new Connector(m_demultiplexors[idx], m_manualClient);
    ...
    void init() throws Exception {
        ...
        m_demultiplexors = new Demultiplexor[m_numDemux];
        for (int i = 0; i < m_numDemux; ++i) {
            m_demultiplexors[i] = new Demultiplexor();
            System.out.println("Start Demultiplexor "
                + i
                + " with thread pool of "
                + m_threads
                + " threads");
            m_demultiplexors[i].start(m_threads);
        }
    }

    void fini() throws Exception {
        for (int i = 0; i < m_demultiplexors.length; ++i) {
            System.out.println("Stop Demultiplexor " + i);
            m_demultiplexors[i].shutdown();
        }
    }
    ...
    private void startTimers() throws InterruptedException {
        ...
        if (m_useTimers && m_serverMode) {
            ...
            m_demultiplexors[0].startTimer(timer);
        }
    ...
    public void parseArgs(String[] argv) throws Exception {
        ...
        for (int i = 0; i < argc; i++) {
            ...
            } else if (argv[i].compareTo("--multiplexors") == 0 ||
                argv[i].compareTo("-d") == 0) {
                m_numDemux = Integer.parseInt(argv[++i]);
    ...
    public static void main(String[] argv) throws Exception {
        TestJProactor test = new TestJProactor();
        test.parseArgs(argv);
        test.run();
    }

    private static void usage() {
        System.out.println("Usage: " + TestJProactor.class.getName() +
            "\n [--mode         [S][A][M]]" +
            "\n [--threads      number]" +
            "\n [--multiplexors number]" +
            "\n [--sessions     number]" +
